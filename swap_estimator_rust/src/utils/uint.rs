...
///
/// * `round_up: true` - `1.1` will round to `2`
/// * `round_up: false` - `1.9` will round to `1`
pub fn mul_div(
    value: u128,
    multiplier: u128,
    divisor: u128,
    ...
    let result = if round_up {
        (value * multiplier + divisor - U256::from(1)) / divisor
    } else {
...
mod tests {
    ...
    #[test]
    fn test_muldiv_rounding() {
        assert_eq!(mul_div(1, 101, 100, true).unwrap(), 2);
        assert_eq!(mul_div(1, 199, 100, false).unwrap(), 1);
    }
}
